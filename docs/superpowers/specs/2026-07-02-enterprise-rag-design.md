...
| **Embedding** | BGE-M3（本地部署，推荐）/ DeepSeek Embedding API（MVP） | — | 1024 维，中英双语，稠密+稀疏 |
...
- `documents` — 文档信息（含 MinIO objectKey）
- `chunks` — 文档切片（含向量 ID 映射）
- `conversations` — 对话记录
- `audit_logs` — 审计日志
